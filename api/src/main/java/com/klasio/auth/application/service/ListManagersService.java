...
public class ListManagersService {

    private final UserRepository userRepository;
    private final TenantNamePort tenantNamePort;

    public ListManagersService(UserRepository userRepository, TenantNamePort tenantNamePort) {
    ...
    public Page<AdminSummary> execute(UUID tenantId, UserStatus status, Pageable pageable) {
        Page<User> userPage = userRepository.findByRole(Role.MANAGER, tenantId, status, pageable);
