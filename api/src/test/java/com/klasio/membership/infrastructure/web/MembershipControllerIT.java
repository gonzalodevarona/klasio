...
/**
 * Integration (controller-slice) tests for the UNLIMITED membership creation flow.
 * Verifies that the HTTP layer correctly handles null purchasedHours/availableHours
 * for UNLIMITED plan memberships and that role-based access is enforced.
 */
@WebMvcTest(controllers = MembershipController.class)
@Import({GlobalExceptionHandler.class, MembershipControllerIT.TestSecurityConfig.class})
class MembershipControllerIT {

    // ------------------------------------------------------------------
    // Security slice configuration
    // ------------------------------------------------------------------
    ...
    static class TestSecurityConfig {
        ...
        /**
         * Mocked DataSource required by TenantContextInterceptor, which calls
         * dataSource.getConnection() during every request to set the RLS tenant context.
         */
        @Bean
    ...
    // ------------------------------------------------------------------
    // Mocked use cases and ports required by MembershipController constructor
    // ------------------------------------------------------------------
    ...
    private UsernamePasswordAuthenticationToken adminAuth() {
        ...
        details.put("userId", ADMIN_ID.toString());
    ...
    private UsernamePasswordAuthenticationToken studentAuth() {
        ...
        Map<String, Object> details = new HashMap<>();
        details.put("userId", studentUserId.toString());
        details.put("tenantId", TENANT_ID.toString());
        auth.setDetails(details);
        return auth;
    }
    ...
    /**
     * Builds an UNLIMITED membership with null purchasedHours and availableHours,
     * as produced by CreateMembershipService for UNLIMITED plans.
     */
    private Membership buildUnlimitedMembership() {
        return Membership.reconstitute(
                ...
                null,            // purchasedHours — must be null for UNLIMITED
                null,            // availableHours — must be null for UNLIMITED
                LocalDate.now(),
    ...
    // ------------------------------------------------------------------
    // Scenario 1: UNLIMITED plan creation → 201 with null hour fields
    // ------------------------------------------------------------------

    @Test
    @DisplayName("POST /memberships with UNLIMITED plan returns 201, modality=UNLIMITED, no hour fields")
    void createUnlimitedMembership_returns201WithNullHours() throws Exception {
        Membership membership = buildUnlimitedMembership();
        when(createMembershipUseCase.execute(any())).thenReturn(membership);
        when(studentNamePort.findFullName(any(), any())).thenReturn(Optional.of("Ana Gomez"));
        when(programNamePort.findName(any(), any())).thenReturn(Optional.of("Adults Unlimited"));
    ...
    // ------------------------------------------------------------------
    // Scenario 2: STUDENT role cannot POST to /memberships → 403
    // ------------------------------------------------------------------

    @Test
    @DisplayName("POST /memberships as STUDENT returns 403 (ADMIN/SUPERADMIN only endpoint)")
    void createMembership_asStudent_returns403() throws Exception {
        String requestBody = """
                {
                  "studentId": "%s",
                  "planId": "%s",
