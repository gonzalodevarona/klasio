...
| `feature/040-self-registration` | RF-40 | RF-40 ✅ (student self-registration via subdomain `{slug}.<root>/register`; `selfRegistrationEnabled` tenant flag V072; unified creation path via `CreateStudentUseCase`; `RegistrationForm` retired) |
...
## Recent Changes
- 040-self-registration: Student self-registration (RF-40): unified self-reg onto `CreateStudentUseCase` (Approach A — single creation path, both admin + self-reg share email+identity uniqueness checks, SYSTEM_ACTOR `createdBy`). New `selfRegistrationEnabled` boolean on `Tenant` (V072, first tenant feature-flag; establishes pattern for RF-38/RF-44). Subdomain entry `https://{slug}.klasio.app/register` (host→slug via `tenantSlugFromHost`). `StudentForm` parametrized (`mode: admin|self`); divergent `RegistrationForm` + `StudentProfileAdapter.createStudentProfile` retired. Superadmin toggle + copy-invite-link in TenantForm. Non-enumerating 409 for public path conflicts. Audit: `TENANT_SELF_REGISTRATION_TOGGLED`.
