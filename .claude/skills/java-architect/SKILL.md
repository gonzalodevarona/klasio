...
- Use `String.format()` or `formatted()` for string composition — never `+` concatenation
- Use polymorphism and inheritance when they genuinely simplify the design (e.g., shared behavior across role types, entity hierarchies) — do not avoid them out of dogma
- Declare all constants in `UPPER_SNAKE_CASE` (e.g., `MAX_RETRY_ATTEMPTS`, `DEFAULT_PAGE_SIZE`)
...
- Concatenate strings with `+` — use `String.format()` or text blocks instead
